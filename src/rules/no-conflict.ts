...
/**
 * Check whether two rule texts express a known conflict pair (e.g. tabs vs
 * spaces, single vs double quotes).
 *
 * Used by no-semantic-duplicate to avoid flagging opposing rules as
 * "semantically similar" — high word overlap from near-identical structure
 * does not equal duplication when the key terms are opposites.
 */
export function isConflictPair(textA: string, textB: string): boolean {
  for (const { a, b } of CONFLICT_PAIRS) {
    a.lastIndex = 0
    b.lastIndex = 0
    const aHitsA = a.test(textA)
    a.lastIndex = 0
    const aHitsB = b.test(textA)
    b.lastIndex = 0
    const bHitsA = a.test(textB)
    a.lastIndex = 0
    const bHitsB = b.test(textB)

    if ((aHitsA && bHitsB) || (aHitsB && bHitsA)) return true
  }
  return false
}
